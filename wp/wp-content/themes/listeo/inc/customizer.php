...
#titlebar.listing-titlebar span.listing-tag { background:  {$maincolor}12; }
...
.icon-box-v3:hover .ibv3-icon {
    background:  {$maincolor};
    box-shadow:  0 3px 8px {$maincolor}50;
}
.radio input[type='radio']:checked + label .radio-label,
...
.listeo-cart-container:hover .mini-cart-button{
    color: {$maincolor};
    background: {$maincolor}1f;
}
.listeo-cart-container .mini-cart-button .badge {
    background: {$maincolor};
}
.transparent-header #header:not(.cloned) .header-widget .woocommerce-mini-cart__buttons a.button.checkout, .listeo-cart-container .woocommerce-mini-cart__buttons a.button.checkout {background: {$maincolor};}
...
if(get_option('listeo_home_slider_background') == 'solid_color'){
    $background_color = get_option('listeo_home_slider_background_color','#fff1e3');
     $custom_css .= '.template-file.main-search-container.plain-color { background-color: '. $background_color .' } ';
}
